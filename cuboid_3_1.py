...
#介绍
print('3d v3.1')
print('1 可以不按回车控制摄像机了')
print('2 画面不会闪了')
print('3 加快了draw()速度 (可能?)')

print('已知特性')
print('1 进入长方体内部画面有问题 只能看到部分棱')
print('2 如果渲染像素的太多 操作会有延迟')
...
while True:
    ...
    #绘图
    draw(coordinate,height)
    print('当前摄像机坐标为',camera)
    ...
    camera_move = msvcrt.getwch()
    ws, ad = (cos(alpha), sin(alpha)), (-sin(alpha), cos(alpha))
    if camera_move == 'w':
    ...
    #控制
        ...
        ws = [step*x for x in ws]
        ad = [step*x for x in ad]
