...
class Mesh:
    ...
    def get_normal(self):
        ...
        for index, face in enumerate(self.f):
            v = (self.v[face[0][1]][0] - self.v[face[0][0]][0], self.v[face[0][1]][1] - self.v[face[0][0]][1], self.v[face[0][1]][2] - self.v[face[0][0]][2])
            u = (self.v[face[0][2]][0] - self.v[face[0][0]][0], self.v[face[0][2]][1] - self.v[face[0][0]][1], self.v[face[0][2]][2] - self.v[face[0][0]][2])
...
class Display:
    def __init__(self, fov=40, size=None, z_near=0.01, z_far=100, bottom_bar_info="", bottom_bar_height=None) -> None:
        import math
        self.ceil = math.ceil
        ...
        self.bottom_bar_info = bottom_bar_info
        if bottom_bar_info == "":
            self.bottom_bar_height = 0
        else:
            self.bottom_bar_height = bottom_bar_height if bottom_bar_height != None else (bottom_bar_info.count("\n") + 1)
        self.refresh()
    ...
    def get_screen_size(self) -> None:
        if self.size == None:
            ...
            w = w // 2 - 2
            h = h - 5 - self.bottom_bar_height
        elif self.size[0] == None:
            w, _ = self.get_terminal_size()
            w = w // 2 - 2
            h = self.size[1] - 5 - self.bottom_bar_height
    ...
    def draw(self):
        ...
        print("".join(output), f"\n{self.bottom_bar_info}", end="")




def render(meshes, display, cam, min_brightness=16, max_brightnss=255, light_dir=(0, 0.5, -0.5*3**0.5), culling=True, faces=True, edges=False, hidden=()) -> None:
    global z_buffer
    display.new_frame()
    z_buffer = [[(-1, "") for _ in range(display.width)] for _ in range(display.height)]
    for mesh in meshes.values():
        if mesh.name in hidden:
            continue
        for face in mesh.f:
...
def keyinput() -> None:
    """Start a new thread to get key input. Access the string variable key by renderer.key. 
    Bool variable pause can also be accessed by renderer.key. Press Shift + P to set it opposite.
    Quit by pressing Shift + Q (With it starting, you can no longer end the program by Ctrl + C.)
    After quitting, renderer.key would remain "Q"
    I don't really recommend you to use it and, rather, you'd better do it yourself."""
    def key_capture():
        global key, pause
    ...
    global key, pause
    globals()["key"], globals()["pause"] = None, False
    Thread(target=key_capture, daemon=True).start()




def calculate_camera_matrix(cam) -> list:
    import math
    trans = math.pi / 180
...
def key_control(cam, display, step):
    """If you use the keyinput function in the module, which I don't recommend, you can call the function have some basic control
    on your camera. But still, I recommend you do it yourself as it'd be more customizable.
    """
    global key
    if key != None:
        if key == "r":
            ...
            cam[5] = calculate_camera_matrix(cam)
            # meshes["Cube_2"].move_mesh((cam[0] + cam[5][2][0], cam[1] + cam[5][2][1], cam[2] + cam[5][2][2]))
